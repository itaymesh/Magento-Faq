 ...
 */
class Studioraz_Faq_Model_Mysql4_Faq_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract
{
     ...
     * 
     * @param int|Studioraz_Faq_Model_Category $category Category to be filtered
     * @return Studioraz_Faq_Model_Mysql4_Category_Collection
     */
    public function addCategoryFilter($category)
    {
        if ($category instanceof Studioraz_Faq_Model_Category) {
            $category = array($category->getId());
        }
        
        $this->getSelect()->join(
            array('category_table' => $this->getTable('studioraz_faq/category_item')),
     ...
     * @return Studioraz_Faq_Model_Mysql4_Faq_Collection Self
     */
    public function addStoreFilter($store)
    {
        ...
        $this->getSelect()->join(
            array('store_table' => $this->getTable('studioraz_faq/faq_store')),
        ...
        if ($this->_previewFlag) {
            ...
            if (count($items)) {
                $select = $this->getConnection()->select()->from(
                    $this->getTable('studioraz_faq/faq_store')
                )->where(
                    $this->getTable('studioraz_faq/faq_store') . '.faq_id IN (?)',
                    $items
                );
