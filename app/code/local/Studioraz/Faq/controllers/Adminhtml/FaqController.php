 ...
 */
class Studioraz_Faq_Adminhtml_FaqController extends Mage_Adminhtml_Controller_Action
{
	/**
	...
	{
		$this->_initAction()
    			->_addContent($this->getLayout()->createBlock('studioraz_faq/adminhtml_item'))
    			->renderLayout();
	}
	...
	public function editAction()
	{
		...
		$model = Mage::getModel('studioraz_faq/faq');
		...
		if ($id) {
			...
			if (!$model->getId()) {
				Mage::getSingleton('adminhtml/session')->addError(
					Mage::helper('studioraz_faq')->__('This FAQ item no longer exists')
				);
				$this->_redirect('*/*/');
				return;
			}
		...
		$this->_initAction()
				->_addBreadcrumb(
					$id
						? Mage::helper('studioraz_faq')->__('Edit FAQ Item')
						: Mage::helper('studioraz_faq')->__('New FAQ Item'),
					$id
						? Mage::helper('studioraz_faq')->__('Edit FAQ Item')
						: Mage::helper('studioraz_faq')->__('New FAQ Item')
				)
				->_addContent(
						$this->getLayout()
								->createBlock('studioraz_faq/adminhtml_item_edit')
								->setData('action', $this->getUrl('adminhtml/faq/save'))
				)
				->_addLeft($this->getLayout()->createBlock('studioraz_faq/adminhtml_item_edit_tabs'));
