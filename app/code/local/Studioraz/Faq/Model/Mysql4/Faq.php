 ...
 */
class Studioraz_Faq_Model_Mysql4_Faq extends Mage_Core_Model_Mysql4_Abstract {
	...
	protected function _construct() {
		...
		$this->_init('studioraz_faq/faq', 'faq_id');
	}
	...
	protected function _getLoadSelect($field, $value, $object) {
		...
		if ($object->getStoreId()) {
			$select->join(
				array('nns' => $this->getTable('studioraz_faq/faq_store')),
	...
	protected function _afterSave(Mage_Core_Model_Abstract $object)
	{
		...
		$this->_getWriteAdapter()->delete($this->getTable('studioraz_faq/faq_store'), $condition);
		foreach ((array) $object->getData('stores') as $store) {
			$storeArray = array ();
			$storeArray['faq_id'] = $object->getId();
			$storeArray['store_id'] = $store;
			$this->_getWriteAdapter()->insert(
				$this->getTable('studioraz_faq/faq_store'), $storeArray
			);
		}
		
		// process faq item to category relation
        $this->_getWriteAdapter()->delete($this->getTable('studioraz_faq/category_item'), $condition);
        foreach ((array) $object->getData('categories') as $categoryId) {
            ...
            $this->_getWriteAdapter()->insert(
                $this->getTable('studioraz_faq/category_item'), $categoryArray
            );
        }
	...
	protected function _afterLoad(Mage_Core_Model_Abstract $object)
	{
		...
		// process faq item to category relation
        $select = $this->_getReadAdapter()->select()->from(
            $this->getTable('studioraz_faq/category_item')
        )->where('faq_id = ?', $object->getId());
