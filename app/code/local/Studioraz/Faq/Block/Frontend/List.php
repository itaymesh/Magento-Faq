 ...
 */
class Studioraz_Faq_Block_Frontend_List extends Mage_Core_Block_Template
{
	 ...
	 *
	 * @param int $pageSize
	 * @return Studioraz_Faq_Model_Mysql_Faq_Collection collection of current FAQ entries
	 */
	public function getFaqCollection($pageSize = null)
	{
		if (!$this->_faqCollection || (intval($pageSize) > 0
			...
			$this->_faqCollection = Mage :: getModel('studioraz_faq/faq')
				->getCollection()
	 ...
	 * 
	 * @return Studioraz_Faq_Model_Mysql4_Category_Collection
	 */
	public function getCategoryCollection()
	{
	    $categories = $this->getData('category_collection');
	    if (is_null($categories)) {
    	    $categories =  Mage::getResourceSingleton('studioraz_faq/category_collection')
    	       ->addStoreFilter(Mage::app()->getStore())
    	       ->addIsActiveFilter();
	 ...
	 * 
	 * @param Studioraz_Faq_Model_Category $category
	 * @return Studioraz_Faq_Model_Mysql4_Faq_Collection
	 */
	public function getItemCollectionByCategory(Studioraz_Faq_Model_Category $category)
	{
	...
	public function getFaqJumplist()
	{
		...
		{
			$this->_faqJumplist = Mage::helper('studioraz_faq/jumplist');
